...
export function SiteHeader({ bookHref = '#book', containerClassName = 'max-w-[1280px] lg:px-8', navBaseHref = '' }: SiteChromeProps) {
    return (
        <header className="sticky top-0 z-50 border-b border-[#DDE4DC]/60 bg-white/95 backdrop-blur">
            <div className={`mx-auto flex items-center justify-between px-4 py-4 ${containerClassName}`}>
                <a href="/" className="flex items-center gap-2 text-[22px] font-semibold tracking-tight">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#DDE4DC]">
                        <Home className="h-5 w-5" />
                    </div>
                    <span>VillaStay</span>
                </a>

                <nav className="hidden items-center gap-8 text-sm font-medium text-[#1F2420] md:flex">
                    {navItems.map((item) => (
                        <a key={item} href={sectionHref(item, navBaseHref)} className="transition hover:text-[#1F5C45]">
                            {item}
                        </a>
                    ))}
                </nav>
                ...
                <div className="flex items-center gap-3">
                    <button className="hidden rounded-full border border-[#DDE4DC] p-2 md:inline-flex" type="button" aria-label="Open menu">
                        <Menu className="h-5 w-5" />
                    </button>
                    <a href={bookHref} className="rounded-xl bg-[#1F5C45] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_10px_24px_rgba(31,92,69,0.22)] transition hover:bg-[#174936]">
                        Book Now
                    </a>
                </div>
            </div>
...
export function SiteFooter({ containerClassName = 'max-w-[1280px] lg:px-8' }: Pick<SiteChromeProps, 'containerClassName'>) {
    return (
        <footer className="border-t border-[#DDE4DC] bg-white">
            <div className={`mx-auto px-4 py-10 ${containerClassName}`}>
                <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-5">
                    {footerColumns.map((column) => (
                        <div key={column.title}>
                            <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-[#1F2420]">{column.title}</h3>
                            <ul className="mt-4 space-y-3 text-sm text-[#6D766E]">
                ...
                <div className="mt-10 flex flex-col gap-4 border-t border-[#DDE4DC] pt-6 text-sm text-[#6D766E] md:flex-row md:items-center md:justify-between">
